...
namespace LightingRewritten
{
    ...
    public class LightingRewritten : MonoBehaviour {
		...
		void Update(){
            ...
            transform.eulerAngles = transform.GetChild(0).eulerAngles;
		...
		private List<LightInteractVertex> raycastAtCollider(PolygonCollider2D collider) {
			...
			for (int i = 0; i < collider.points.Length; i++) {
                ...
                try
                {
                    worldPoint = collider.transform.TransformPoint(collider.points[i]);
                }
                catch (Exception e)
                {
                    Debug.Log("Error raycasting to " + collider.transform.tag + "'s " + i + " index point out of " + collider.GetTotalPointCount() + " points (array size " + collider.points.Length + ")");
                }
